	...
	public static void resolveMessage(Context context,String message, boolean isOffline) {
		...
		try {
			...
			if (!StringUtil.isEmpty(recMsg.getSendId())
				...
				if (MessageManager.getInstance(context)
				...
				// 保存存用户信息
				PersonInfoManager.getInstance(context)
...
//							.saveGroupInfo(recMsg);
				...
				case SendMessageItem.TYPE_SEND_SUCCESS:// 消息发送成功
					...
					if (primaryId != -1) {
						...
						Intent intent = new Intent(
								IMConstant.IM_MESSAGE_SEND_SUCCESS_ACTION);
	 ...
	 *            你内容
	 */
	private static void setNotiType(Context context,int iconId, String contentTitle,
